...
impl CollateralEntry {
    ...
    pub fn withdraw(&mut self, amount: u64) -> Result<(), String> {
        if self.amount >= amount {
            ...
            Err("Not enough collateral".to_string())
        }
    }

    // New method to check collateralization
    pub fn check_collateralization(&self, current_icp_price: u64) -> Result<(), String> {
        let total_value = self.amount * current_icp_price;
        let required_collateral = total_value * 150 / 100; // Example: 150% of the iUSD issued

        if total_value < required_collateral {
            Err("Collateral below required minimum, additional collateral needed".to_string())
        } else {
            Ok(())
        }
    }
